...
export class FeatureList {
    ...
    async getProgress(feature, type) {
        try {
    ...
    filterFeatures() {
        return this.features.filter(feature => {
            if (this.currentFilter.type && this.currentFilter.type !== 'all') {
                if (feature.type !== this.currentFilter.type)
                    return false;
            }
            if (this.currentFilter.status) {
                if (feature.status !== this.currentFilter.status)
                    return false;
            }
            if (this.currentFilter.category) {
                if (feature.category !== this.currentFilter.category)
                    return false;
            }
            return true;
        });
    ...
    attachCardEventListeners(card, feature) {
